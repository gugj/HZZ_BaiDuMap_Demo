...
public class SearchView extends LinearLayout implements OnClickListener {
	...
	public SearchView(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		LayoutInflater.from(context).inflate(R.layout.custom_auto_complete_view, this);
	...
	public class EditChangedListener implements TextWatcher {
		...
		public void onTextChanged(CharSequence s, int start, int before, int count) {
			if (!"".equals(s.toString())) { //输入框EditText的内容不为空----显示自动补全搜索记录
			...
			} else { //输入框EditText的内容为空-----显示历史搜索记录
				ivDelete.setVisibility(GONE);
